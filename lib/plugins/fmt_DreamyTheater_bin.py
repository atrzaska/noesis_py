...
def objBinLoadModel(data, mdlList):
    ...
    global texList, matList, textureIdToName, textureIds, boneData_Array, boneList, kfBones, baseName, bAnimInfoName, bAnimInfoRot
    ...
    baseName = rapi.getExtensionlessName(rapi.getLocalFileName(rapi.getLastCheckedName()))[:-4]
...
def load_BinModel(bs):
    ...
    if boneTableOff1_Array[0] != 0:
        ...
        if boneData_Array != []:
            ...
            for i in range(len(boneData_Array[0])):
        ...
        for a in range(materialCount):
            ...
            for b in range(8):
                ...
                if texType == 0xf1 and b == 0:
                    material.setTexture(textureIdToName[textureId])
                elif texType == 0xf2:
                    material.setNormalTexture(textureIdToName[textureId])
                elif texType == 0xf3:
                    material.setSpecularTexture(textureIdToName[textureId])
                elif texType == 0x3f9:
                    material.setEnvTexture(textureIdToName[textureId])
                else:
                    pass
...
def load_binTex(binTex, texList):
    ...
    for i in range(texCount):
        ...
        name = textureIdToName[textureIds[i]]
...
def load_binTexDB(binTexDB):
    ...
    for i in range(texCount):
        ...
        textureIdToName[texNameID] = texName
